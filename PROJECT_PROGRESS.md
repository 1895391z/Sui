...
- Coal 原题已完成退出码2的单位澄清验收；将流量修正为1000 kg/h后，1400°C live run
  成功，并保留 `engineering_validation_status=limited` 与973.15°C外推警告。
- 最终演示已按固定顺序完成整套连续彩排：一条预期澄清和三个成功案例均达到验收条件，
  证据保存在 `cases/runtime/final_demo/`。
...
- 三场景默认工况、参数矩阵、JSON live run 和最终演示连续彩排均已完成；
- 可选 LLM 解析尚未实现。
...
三条代表性自然语言成功命令及一条澄清式失败命令已固定在
[最终演示彩排方案](docs/final_demo_rehearsal.md)，并已从无 HYSYS 进程状态完成整套严格串行
彩排。68项离线测试通过，三个成功案例均清理自有进程，所有 seed 哈希不变，stdout/stderr
和元数据证据完整。可选 LLM 只作为后续增强，不能替代确定性安全校验。
